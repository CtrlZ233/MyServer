...
namespace Utils{
    ...
    Log::Log(std::string logDir, int maxWaitSeconds) : maxWaitSeconds_(maxWaitSeconds){
        ...
        sprintf(curTime, "LOG-%02d-%02d-%02d_%02d:%02d:%02d.log", ptminfo->tm_year + 1900, ptminfo->tm_mon + 1,
    ...
    void Log::Append(const std::string & s, const std::string & fileName, const std::string & line, 
                     const std::string functionName) {
        // printf("Append\n");
        time_t ts=time(NULL);
        std::string res = GetCurrentDateTime() +  " " + fileName + " line: " + line;
        res += " " + functionName + ": " + s +"\n";
        if (!isShutDown_) {
            std::lock_guard<std::mutex> lock(currentBufferMutex_);
